...
const heroSlides = computed(() => {
  ...
  if (slides.length > 0) {
    return slides.map(s => ({
      ...
      badge: s.badge_text || 'New Season Collection',
    }))
  }
  return [{
...
<template>
  <div>
    ...
    <section class="relative overflow-hidden bg-gradient-to-br from-primary-900 via-primary-800 to-primary-700">
      ...
      <div v-if="!heroSlides[currentSlide]?.bg_image" class="absolute inset-0 opacity-10">
      ...
      <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 relative z-10">
        <template v-for="(slide, index) in heroSlides" :key="index">
          <div v-show="index === currentSlide" class="flex items-center gap-12">
            <!-- Text Content -->
            <div class="flex-1" :class="slide.feature_image && slide.image_position === 'left' ? 'order-2' : ''">
              ...
              <div class="flex flex-wrap gap-4">
                <RouterLink :to="slide.link" class="btn bg-white text-primary-800 hover:bg-primary-50 shadow-xl shadow-black/10 font-semibold">
                  {{ slide.cta }}
            ...
            <div v-if="slide.feature_image" class="flex-1 hidden md:flex items-center justify-center"
              :class="slide.image_position === 'left' ? 'order-1' : ''">
        ...
        <div v-if="heroSlides.length > 1" class="flex gap-2 mt-8">
          <button v-for="(_, i) in heroSlides" :key="i"
            ...
            :class="i === currentSlide ? 'bg-white w-8' : 'bg-white/40 hover:bg-white/60'">
          </button>
        </div>
      </div>
      <!-- Decorative wave -->
    ...
    <!-- Features Strip -->
    <section v-if="featureItems.length" class="bg-neutral-50 py-6">
      <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div v-for="(item, idx) in featureItems" :key="idx" class="flex items-center gap-3 p-3">
            <div class="w-10 h-10 rounded-xl bg-primary-50 flex items-center justify-center flex-shrink-0 text-xl">
              {{ item.icon }}
            </div>
            <div>
              <p class="text-sm font-semibold text-neutral-800">{{ item.title }}</p>
        ...
        <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          <div v-for="product in featuredProducts.slice(0, 8)" :key="product.id"
            ...
            <div class="aspect-square bg-neutral-50 relative overflow-hidden">
              ...
              <div v-if="product.sale_price" class="absolute top-3 left-3 bg-accent-500 text-white text-xs font-bold px-2.5 py-1 rounded-lg">
                SALE
              </div>
            </div>
            <div class="p-4">
              ...
              <div class="flex items-baseline gap-2">
                <span class="text-lg font-bold text-primary-700">{{ formatPrice(product.sale_price || product.regular_price) }}</span>
                <span v-if="product.sale_price" class="text-sm text-neutral-400 line-through">{{ formatPrice(product.regular_price) }}</span>
              </div>
            </div>
          </div>
        </div>
